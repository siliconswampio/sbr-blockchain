...
const ethash_1 = __importDefault(require("@sbr/ethash"));
...
class Blockchain {
    ...
    async _init(genesisBlock) {
        let dbGenesisBlock;
        try {
            const genesisHash = await this.dbManager.numberToHash(new sbr_util_1.BN(0));
        ...
        if (!dbGenesisBlock) {
            ...
            const dbOps = [];
            dbOps.push(helpers_1.DBSetTD(genesisBlock.header.difficulty.clone(), new sbr_util_1.BN(0), genesisHash));
            helpers_1.DBSetBlockOrHeader(genesisBlock).map((op) => dbOps.push(op));
            helpers_1.DBSaveLookups(genesisHash, new sbr_util_1.BN(0)).map((op) => dbOps.push(op));
    ...
    async cliqueUpdateSignerStates(signerState) {
        ...
        dbOps.push(helpers_1.DBOp.set(operation_1.DBTarget.CliqueSignerStates, sbr_util_1.rlp.encode(formatted)));
        await this.dbManager.batch(dbOps);
    ...
    async cliqueUpdateVotes(header) {
        ...
        if (header && !header.coinbase.isZero()) {
            ...
            for (let round = 1; round <= 2; round++) {
                ...
                const lastEpochBlockNumber = header.number.sub(header.number.mod(new sbr_util_1.BN(this._common.consensusConfig().epoch)));
                const limit = this.cliqueSignerLimit();
        ...
        const lastBlockNumber = (_a = blockSigners[blockSigners.length - 1]) === null || _a === void 0 ? void 0 : _a[0];
        if (lastBlockNumber) {
            const lastEpochBlockNumber = lastBlockNumber.sub(lastBlockNumber.mod(new sbr_util_1.BN(this._common.consensusConfig().epoch)));
        ...
        dbOps.push(helpers_1.DBOp.set(operation_1.DBTarget.CliqueVotes, sbr_util_1.rlp.encode(formatted)));
        await this.dbManager.batch(dbOps);
    }
    /**
    ...
    async cliqueUpdateLatestBlockSigners(header) {
        ...
        dbOps.push(helpers_1.DBOp.set(operation_1.DBTarget.CliqueBlockSigners, sbr_util_1.rlp.encode(formatted)));
        await this.dbManager.batch(dbOps);
    }
    /**
    ...
    async _putBlockOrHeader(item) {
        await this.runWithLock(async () => {
            ...
            const currentTd = { header: new sbr_util_1.BN(0), block: new sbr_util_1.BN(0) };
            let dbOps = [];
            ...
            let parentTd = new sbr_util_1.BN(0);
            if (!block.isGenesis()) {
